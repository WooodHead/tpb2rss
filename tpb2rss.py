...
def main_program(soup):
	...
	for i in range(len(tables) / 4):
		...
		xml += "\nLeechers: " + str(str(tables[position + 3]).split(">")[1]).split("<")[0] + "</description>\n\t\t\t</item>"
		position += 4
...
if len(sys.argv) == 2:
	...
	print "Usage:", sys.argv[0], "INPUT_FILE or SEARCH_TERM [OUTPUT_FILE]"